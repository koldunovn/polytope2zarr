#!/bin/bash
#SBATCH --job-name=story_cont_r4_3d
...
#SBATCH --output=story_cont_r4_3d_%j.out
#SBATCH -e story_cont_r4_3d_%j.err

ulimit -s 204800
ulimit -c 0

echo "Model: ifs-fesom, Activity: story-nudging, Experiment: cont, Realization: 4 (3D data)"
...
for var in "${O3D_VARS[@]}"; do
    ...
    for year in $(seq $YEAR_START $YEAR_END); do
        ...
        python download_to_zarr.py --stream $STREAM --year-range $year $year \
            ...
            --activity $ACTIVITY --experiment $contERIMENT \
