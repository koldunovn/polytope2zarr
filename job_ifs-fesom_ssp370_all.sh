...
#SBATCH --output=ifsf_ssp_all_%j.out
#SBATCH -e ifsf_ssp_all_%j.err

# limit stacksize and core file size
ulimit -s 204800
ulimit -c 0

echo "Model: ifs-fesom, Experiment: ssp3-7.0 (ALL monthly sfc and o2d variables)"
...
for var in "${SFC_VARS[@]}"; do
    ...
    python download_to_zarr.py --stream $STREAM --year-range $YEAR_START $YEAR_END \
        --param $param --levtype sfc --activity $ACTIVITY --experiment "$EXPERIMENT" --model $MODEL
done
...
for var in "${O2D_VARS[@]}"; do
    ...
    echo "Downloading $name (param $param)..."
    python download_to_zarr.py --stream $STREAM --year-range $YEAR_START $YEAR_END \
        --param $param --levtype o2d --activity $ACTIVITY --experiment "$EXPERIMENT" --model $MODEL
done
